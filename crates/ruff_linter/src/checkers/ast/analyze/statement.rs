...
use crate::rules::{
    airflow, brt_lint, flake8_bandit, flake8_boolean_trap, flake8_bugbear, flake8_builtins,
...
pub(crate) fn statement(stmt: &Stmt, checker: &mut Checker) {
    match stmt {
        ...
        Stmt::FunctionDef(
            ...
            if checker.enabled(Rule::FunctionNamedA) {
                if let Some(diagnostic) = brt_lint::rules::function_named_a(name) {
                    checker.diagnostics.push(diagnostic);
                }
            }
